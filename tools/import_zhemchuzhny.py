...
def tables(rows: list[dict]) -> dict:
    ...
    return {
        ...
        "tables": {
            ...
            # Трудовое обучение ставится сдвоенным уроком: завуч Жемчужненской
            # 23.09.2026 — «в шестых и седьмых должно стоять парочками».
            # Норма это разрешает (п. 65 ССЭТ, труд — без условий), а сработает
            # флаг только там, где часов чётное число: в 6–7 их два, в 5, 8 и 9
            # по одному, и пара там невозможна.
            "subjects": [{"предмет": s,
                          "кабинет": PLAN_ROOM.get(TO_PLAN.get(s, s), "обычный"),
                          "только в нём": False,
                          "всегда парой": s == "Трудовое обучение"} for s in subjects],
