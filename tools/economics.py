...
TEAMS = {
    # Влад один и без оклада — это не гипотеза, а текущее положение дел.
    # Оклада нет, но юрлицо, бухгалтер и сервер появляются в тот день, когда
    # первая школа платит деньги: взять оплату мимо юрлица нельзя.
