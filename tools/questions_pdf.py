...
NOT_KNOWN = [
    ("Сочетания профилей — какие дети вместе", "Вы писали про двенадцать сочетаний: "
     "ребёнок на повышенной истории и английском не может быть на двух уроках сразу, "
     "а химия у одних детей и общество у других — могут идти параллельно. Теперь "
     "система это умеет, но ей нужно знать сами сочетания: для каждого профильного "
     "класса — как называется группа детей и какие предметы она берёт на повышенном "
     "уровне. Например: 10«А» — «физмат» (математика, физика), «гуманитарии» "
     "(история, общество, английский). И отдельно: бывают ли группы, собранные "
     "из детей разных классов — например, повышенная химия для части 11«А» и "
     "части 11«Б» вместе? Если да, назовите такие группы."),
    ("Пик нагрузки — вторник, четверг и пятница?", "Мы поставили пик так, как вы "
     "сказали. П. 94 ССЭТ называет вторник, среду и (или) пятницу; расхождение "
     "в четверге система отмечает, но расписание составлено по-вашему. Если "
     "хотите по норме — переключим одной кнопкой."),
...
def build(school: str, out: Path) -> Path:
    ...
    pdf.body("Ещё два места, где система сама знает, что вышло неидеально: "
             "у 10«А» и 11«А» физкультура стоит два дня подряд. Три занятия при "
