...
class Subject:
    ...
    # Ставится ли предмет ТОЛЬКО парой уроков подряд.
    #
    # Трудовое обучение так и стоит почти везде: пока класс дойдёт до мастерской,
    # переоденется и разложит инструмент, одного урока не остаётся. Норма это
    # разрешает (п. 65 ССЭТ № 525), но разрешение — не то же самое, что «всегда
    # так делаем»: последнее знает только школа, поэтому это ввод, а не норма.
...
class SchoolClass:
    ...
    advanced: bool = False
    # Кабинет класса. У школы он почти всегда есть и записан у завуча в той же
    # таблице, что смены: «5А — 36». Без него класс кочует по всему зданию.
...
class Teacher:
    ...
    # Сколько уроков в день этому учителю ставить нельзя больше.
    #
    # Прямой нормы на это нет, но ограничение живое: система старается собрать
    # уроки учителя в меньшее число дней, чтобы он реже ездил в школу, — и без
    # верхней границы может собрать восемь уроков подряд. Для учителя это
    # тяжелее, чем лишний выход. Пусто = ограничения нет.
...
# СТЫК СМЕН. Учитель с последним уроком первой смены на 6-м и первым второй
# на 9-м ждёт в школе два урока — это такая же форточка, как внутри смены.
# А перерыв в три урока и больше — уже не ожидание: учитель уходит и приходит
# снова, и цена этому — «день в обе смены», а не окна. Одна граница для
# солвера и валидатора: иначе солвер прятал ожидания на стыке, где они ему
# ничего не стоили, а валидатор их не видел (25.09.2026, Жемчужненская:
# коротких ожиданий на стыке 26 → 61 часов после ужесточения норм).
SHIFT_SEAM_WAIT = 2
