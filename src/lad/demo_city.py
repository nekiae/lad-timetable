...
def _rooms_for(load: pd.DataFrame, subjects: pd.DataFrame, periods: int, days: int,
    ...
    for kind, need in sorted(hours.items()):
        ...
        if kind == "обычный":
            # Обычных кабинетов — по одному на класс. Расчёт «по средней доле
            # уроков в спецкабинетах» выглядит точнее, но он неверен: среднее
            # сходится, а расписание упирается в ПИК — час, когда в спецкабинетах
            # оказалось меньше классов, чем в среднем. Чем больше школа, тем
            # вероятнее такой час.
            # Замерено 26.08.2026 на гимназии в 28 классов: при 20 обычных
            # кабинетах (столько давало среднее) расписание не находилось
            # ни за 2,5, ни за 10 минут; при 28 — OPTIMAL за 9 секунд, причём
            # со ВСЕМИ нормами в жёстком режиме.
